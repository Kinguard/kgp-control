...
int ns_set_ssl_priv_key(struct ns_server *, const char *key);
...
int ns_set_ssl_priv_key(struct ns_server *server, const char *key){
#ifdef NS_ENABLE_SSL
...
int ns_set_ssl_cert(struct ns_server *server, const char *cert) {
  ...
  /*
  if( SSL_CTX_use_certificate_chain_file(server->ssl_ctx, cert) == 1 )
  {
	  return 0;
  }
	*/
...
#define MONGOOSE_IDLE_TIMEOUT_SECONDS 180
#endif
...
const char *mg_set_option(struct mg_server *server, const char *name,
  ...
  *v = mg_strdup(value);
  ...
  } else if (ind == SSL_PRIVATE_KEY) {
	int res = ns_set_ssl_priv_key(&server->ns_server, value);
